...
class SelectResponseParserSpec extends Specification{
  ...
  "SelectResponseParser" >> {
    "must parse Json with no item" >> {
      ...
      val response = new SelectResponse(jsonStr, "http://localhost:10041/d/select?table=Site&someQuery")
      ...
      response.as[Person].size must beEqualTo(0)
      response.requestUri must beEqualTo("http://localhost:10041/d/select?table=Site&someQuery")
    }
    "must parse Json with some items" >> {
      ...
      val items = response.as[Person]
      items.length must beEqualTo(2)
      items.head._id must beEqualTo(1)
      items.head._key must beEqualTo("alice")
      items.head.age must beEqualTo(18)
    }
    ...
    "must parse Json with some items when hits is different from length of array.(ex: limit parameter used)" >> {
      ...
      response.as[Person].length must beEqualTo(2)
    }
    ...
    "must parse Json with some items, and multi label drilldowns" >> {
      ...
      val response = new SelectResponse(jsonStr, "http://localhost:10041/d/select?table=Site&someQuery")
      response.drillDowns("genre").toMap.size must beEqualTo(2)
      response.drillDowns("user")(1000) must beAnInstanceOf[DrillDown]
      response.drillDowns("user")(1000).nsubrecs must beEqualTo(Some(2))
    }

    "must convert items in JSON to Map" >> {
      val jsonStr = """[[0,1430732073.06866,0.000226974487304688],[[[2],[["_id","UInt32"],["_key","ShortText"],["age","Int8"]],[1,"alice",18], [2,"john",20]]]]"""

      val response = new SelectResponse(jsonStr, "http://localhost:10041/d/select?table=Site&someQuery")
      val items = response.asMap
      items must beAnInstanceOf[Seq[Map[String, Any]]]
      items.length must beEqualTo(2)
      items.head("_id")must beEqualTo(1)
      items.head("_key") must beEqualTo("alice")
      items.head("age") must beEqualTo(18)
