...
class CollectionConverterSpec extends Specification {
  "CollectionConverter should" >> {
    "convert Map to case class" >> {
      ...
      val person = CollectionConverter.map2class[PersonSimple](typeOf[PersonSimple], personMap)
    ...
    "convert Map with vector column to case class" >> {
      ...
      val person = CollectionConverter.map2class[PersonWithSeq](typeOf[PersonWithSeq], personMap)
    ...
    "convert nested Map with vector column to case class." >> {
      ...
      val person = CollectionConverter.map2class[PersonWithJob](typeOf[PersonWithJob], personMap)
